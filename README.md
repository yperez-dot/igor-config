...
The webhook accepts text and file attachments from the explicit allowlist. Word, Excel, PowerPoint, PDF, CSV, and text are extracted; photos/image files are sent to Grok vision; videos attach still frames/thumbnails because Grok cannot watch raw video. **Igor takes it all:** do not add a channel path that silently drops an input type the team already sends. Audit events stay metadata-only. Bounded recent turns for that chat are stored separately so Grok can keep continuity (last 16 turns in the prompt, 40 retained; 1,500 characters for text, 12,000 for extracted files). Identity comes from `src/identity.js` plus `memory/standing.md` (the curated OpenClaw memory pack). Bounded recent turns cover the current chat; `memory_search` / `memory_remember` cover standing THEI facts. Follow [the Telegram cutover runbook](docs/TELEGRAM-CUTOVER.md) and begin with a separate test bot. A Telegram bot supports only one webhook, so pointing the current production bot at v2 is a cutover, not a shadow-mode test.

See [the migration inventory](docs/MIGRATION-INVENTORY.md) for the feature-parity order and cutover gates. For the fastest path to production, see [the ASAP cutover plan](docs/ASAP-CUTOVER.md). Live API access (GHL, Notion, GitHub, Netlify, Facebook Ads, Tavily, OliComm, MedicarePro, email, sales sheet, Google Calendar, Drive, and Gmail) is documented in [system credentials](docs/SYSTEMS.md). OAuth setup is in [Google Calendar](docs/GOOGLE-CALENDAR.md) and [Google Workspace](docs/GOOGLE-WORKSPACE.md). The authenticated `GET /v1/systems` and `GET /v1/migration/status` endpoints expose current connection and migration status. Standing memory is documented in [MEMORY.md](docs/MEMORY.md).
