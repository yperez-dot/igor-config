...
- **Agent Pulse is live on Railway v2 (locked 2026-08-31):** Monday 8:00 AM ET `v2-agent-pulse` / `agent_pulse_weekly`. THE Health Experts Insider from `info@`. Scan `theiagentpulse@gmail.com`. Industry Pulse is the old name for this same email — keep `v2-industry-pulse` off. If a “No v2 handler is registered for workflow: agent_pulse_weekly” alert fires, the worker is missing the Pulse handler — Telegram catch-up is `run_agent_pulse` after that deploy. Issue numbers from July 13, 2026 Issue #4 (Monday Aug 31, 2026 = Issue #11).
