...
- **Agent Pulse is live on Railway v2 (locked 2026-08-31):** Monday 8:00 AM ET `v2-agent-pulse` / `agent_pulse_weekly`. THE Health Experts Insider from `info@` via Gmail SMTP. THEI does not use SendGrid. Railway is Pro (Yahoska 2026-08-31) so outbound SMTP is allowed — after a plan upgrade, **redeploy igor-config** or the old Hobby firewall stays. Scan `theiagentpulse@gmail.com`. `/health` `pulseReady` / `pulseBlockers` is the send-path check — `imap: connected` is info@ only. Worker boot and heartbeat 🚨 the full blocker list (PULSE_IMAP_PASS, SMTP for info@, recipients, XAI_API_KEY) instead of discovering one missing secret per catch-up. Set `PULSE_IMAP_PASS` on **both** services. Worker boot queues this week’s issue if it has not sent. Industry Pulse is the old name for this same email — keep `v2-industry-pulse` off. Do not queue another catch-up while `pulseReady` is false. Issue numbers from July 13, 2026 Issue #4 (Monday Aug 31, 2026 = Issue #11).
