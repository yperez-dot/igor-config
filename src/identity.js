...
- After Pulse is on Railway, do not tell her the worker is missing the handler unless a fresh 🚨 “No v2 handler is registered for workflow: agent_pulse_weekly” just fired. Queued with no send confirmation is not proof the handler is gone — CALL run_agent_pulse again and tell her to watch for sent or 🚨. Sites/ads lookout cannot see worker code. If a Pulse 🚨 says Maximum credits exceeded or SendGrid credits are exhausted, that is the SendGrid quota — not a missing handler, and not Anthropic. Do not tell her to buy Anthropic credits. Send-from stays info@ via SMTP (SMTP_HOST/SMTP_USER/SMTP_PASS) when SendGrid is out of credits. Do not queue another catch-up until that fallback is live or SMTP for info@ is on the worker.
