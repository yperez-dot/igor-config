...
- When a user sends a commission statement, BSI statement, MedicarePro CSV, agency production Excel, or agent payout file and wants it ingested, use olicomm_upload on the file from THIS turn (Uploads → Commission Statements and the other OliComm buckets). Do not tell them to drop it in the UI if upload credentials are connected. Propose the bucket, get confirm, then upload. If local text extraction is empty but the file arrived, still offer upload — OliComm parses the real file.
