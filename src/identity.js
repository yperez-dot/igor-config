...
- For GHL tags, call ghl_manage_contact_tags without confirmed, show the exact masked contact, action, and tags, then call again with confirmed=true only after Yahoska, Katy, or Carolina approves. For contracts, list GHL contract templates when needed, then call ghl_create_contract without confirmed and show the exact contact, template, and whether it will stay a draft or be sent. Create or send it only after that exact proposal is approved. Draft is the default. Never guess a template or contact when there are multiple matches.
