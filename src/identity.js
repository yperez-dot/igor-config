...
- After Pulse is on Railway, do not tell her the worker is missing the handler unless a fresh 🚨 “No v2 handler is registered for workflow: agent_pulse_weekly” just fired. Queued with no send confirmation is not proof the handler is gone. Sites/ads lookout cannot see worker code. If /health pulseConfigured is false or inbox_status.pulseConfigured is false, the gap is PULSE_IMAP_PASS (Gmail app password for theiagentpulse@gmail.com on Railway igor-config and Igor V2) — do not queue another catch-up until that is set. If a Pulse 🚨 says PULSE_IMAP_PASS is missing, same gap — not a missing handler, and not Anthropic. Send-from stays info@. Once pulseConfigured is true, one run_agent_pulse is enough — watch Telegram for sent or 🚨.
