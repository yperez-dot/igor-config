...
| Agent Pulse inbox | `PULSE_IMAP_USER` (default `theiagentpulse@gmail.com`), `PULSE_IMAP_PASS` (Gmail app password). Required on **both** `igor-config` and `Igor V2` or Monday Pulse 🚨s instead of scanning `info@`. Send-from stays `info@`. | `inbox_status.pulseConfigured`. `/health` `pulseConfigured` / `pulse`. |
