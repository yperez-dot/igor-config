...
| Google Drive + Gmail | `GOOGLE_WORKSPACE_REFRESH_TOKEN`; reuses the Calendar client id/secret unless `GOOGLE_WORKSPACE_CLIENT_ID` and `GOOGLE_WORKSPACE_CLIENT_SECRET` are set. See `GOOGLE-WORKSPACE.md`. | Search/read Drive, search/read Gmail, and create a confirmed Gmail draft. No Gmail send tool. |
