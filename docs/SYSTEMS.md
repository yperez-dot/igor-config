...
| Email | `SENDGRID_API_KEY` (used first when set). `FROM_EMAIL` defaults to `info@healthexps.com`. SMTP (`SMTP_HOST`, `SMTP_USER`, `SMTP_PASS` — Gmail app password for `info@`) is the fallback when SendGrid returns `Maximum credits exceeded`. Optional `EMAIL_ALLOWED_RECIPIENTS` | `send_internal_email` (Yahoska standing-approved). `ghl_stale_leads` auto-emails a PHI-light CSV. Agent Pulse send-from stays `info@`. |
...
| Leadership inbox | `HEARTBEAT_IMAP_USER`, `HEARTBEAT_IMAP_PASS`. Pulse inbox: `PULSE_IMAP_USER` (default `theiagentpulse@gmail.com`), `PULSE_IMAP_PASS` | `inbox_status` (no message bodies) |
| Standing memory | none (files in `memory/` + Postgres `agent_memories`) | `memory_search`, `memory_remember`. See [MEMORY.md](MEMORY.md). |
| Lookout / crons | none | `run_lookout` (ads token + public sites — not OliComm). Live jobs: `v2-site-uptime` every 5 min (healthexps.com / Hub never-down), `v2-igor-heartbeat` every 30 min (ads token), `v2-sales-tracker-sync` Monday 7:00 AM ET, `v2-carrier-inbox-digest` daily 7:00 AM ET, `v2-agent-pulse` Monday 8:00 AM ET. `list_schedules` (live + shadow catalog). Telegram catch-up: `run_sales_tracker_sync`, `run_agent_pulse`. Industry Pulse is the old name for Agent Pulse — keep `v2-industry-pulse` off. |
