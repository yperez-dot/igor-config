...
| Email | `SMTP_HOST`, `SMTP_USER`, `SMTP_PASS` (Gmail app password for `info@healthexps.com`). `FROM_EMAIL` defaults to `info@healthexps.com`. THEI does not use SendGrid. Optional `EMAIL_ALLOWED_RECIPIENTS` (Yahoska + Katy + info@ are always allowed). Optional `TELEGRAM_KATY_USER_ID` so Telegram knows Katy | `send_internal_email` (Yahoska and Katy standing-approved). `ghl_stale_leads` auto-emails a PHI-light CSV to the person in the chat. Agent Pulse send-from stays `info@`. |
