...
| VA Telegram check-in | **Live on Igor V2 only** (`VA_CHECKIN_ENABLED=true`, default off) | Monday 9:00 AM ET DM to Yahoska, Katy, and Carolina with their Notion Open projects + monthly todos (`va_checkin`). Must not run on igor-config / Igor V2 Test. Igor writes Notion from their Telegram replies. Tuesday 3:00 PM ET one nudge if silent. Katy/Carolina have no Charlie. |
