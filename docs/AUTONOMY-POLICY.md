...
Connecting Google Calendar was approved so Igor can view and book team calendars (Yahoska, Katy, Carolina). He still signs in as `yperez@healthexps.com`. Katy already shares her calendar with that account (Yahoska 2026-09-01). Carolina still needs to share hers (Make changes to events). Each create, reschedule, or cancel still needs an in-chat confirmation before it writes to the calendar.
