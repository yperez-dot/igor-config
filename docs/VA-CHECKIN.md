...
- Do **not** intercept referral thank-you card lists (clients who send referrals, still need a thank-you). Those go to the Referral Thank-Yous database. Never create Weekly Focus or Monthly Todos for that list. Telegram routing: `Add to Referral Thank-Yous` / log-referral-thank-you create runs **before** lead-reminder / “That lead was removed,” so a previously removed lead name (e.g. Maria Lopez) still gets a tracker row.
