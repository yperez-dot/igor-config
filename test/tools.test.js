...
test("GHL tools appear only when GHL_API_TOKEN is set", () => {
  ...
  assert.equal(names({ GITHUB_TOKEN: "gh" }).includes("update_hub_ticker"), false);
...
test("legacy update_hub_ticker executor is disabled for every caller", async () => {
  const result = await executeTool("update_hub_ticker", { slower: true, confirmed: true }, {
    environment: { GITHUB_TOKEN: "gh", TELEGRAM_YAHOSKA_USER_ID: "888" },
    senderId: "888"
  });
  assert.equal(result.status, "skipped");
  assert.match(result.error, /Direct Hub ticker publishing is disabled/);
});
